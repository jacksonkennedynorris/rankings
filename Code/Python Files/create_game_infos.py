...
## Football
   ...
   #directory = os.path.normpath("C:\Users\Will\Desktop\Summer Research 2019\HS Sports Data\Football/" + year + "/HTML") ## Open and traverses through each file
    ...
    if not os.path.exists(season.get_year_path() + "/game_infos"):  
        os.makedirs(season.get_year_path() + "/game_infos")
   
    for file_name in os.listdir(season.get_year_path() + "/game_infos"): 
        os.remove(os.path.join(season.get_year_path() + "/game_infos", file_name))
    ...
    #         # Write to file
    #         f = open('../MATLAB/' + season.sport + '/Data/' + str(season.year) + '/game_infos/' + filename, 'w')
    #         f.write(str(season.year))
    #         f.write('\n')
    #         f.write(week_num)
    #         f.write('\n')
            
    #         f.write('WHY ISN"T IT WRITING')
